...
func (a *App) buildQueryDBToolDefinition() GmailToolDefinition {
	...
	return GmailToolDefinition{
		...
		Description: strings.Join([]string{
			...
			fmt.Sprintf("The cache is populated by a full sync of roughly the last %d days of mail using users.messages.list plus users.messages.get(format=full), then kept updated by incremental users.history.list(startHistoryId=...) syncs every 5 minutes and before freshness-sensitive chat requests; if Gmail returns history 404, the app falls back to a fresh %d-day full sync.", emailSyncRecentWindowDays, emailSyncRecentWindowDays),
